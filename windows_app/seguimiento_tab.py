...
class SeguimientoTab(QWidget):
    ...
    def refresh_data(self):
        ...
        try:
            ...
            self.lote_combo.blockSignals(True)  # Evitar que se disparen señales durante la actualización
            self.historico_lote_combo.blockSignals(True)
            self.grafico_lote_combo.blockSignals(True)
            
            self.lote_combo.clear()
            self.historico_lote_combo.clear()
            self.grafico_lote_combo.clear()
            
            if success and isinstance(lotes, list) and lotes:
                # Agregar lotes desde la API
                for lote in lotes:
                ...
                # Si no hay lotes o hay un error, mostrar datos de ejemplo
                print("No se pudieron cargar lotes desde la API. Mostrando datos de ejemplo.")
            ...
            # Reactivar señales después de la actualización
            self.lote_combo.blockSignals(False)
            self.historico_lote_combo.blockSignals(False)
            self.grafico_lote_combo.blockSignals(False)
            
            # Seleccionar el primer lote si hay alguno disponible
            if self.lote_combo.count() > 0:
                self.lote_combo.setCurrentIndex(0)
                self.historico_lote_combo.setCurrentIndex(0)
                self.grafico_lote_combo.setCurrentIndex(0)
                # Actualizar la tabla con los datos del lote seleccionado
                self.on_lote_changed()
            else:
                # Si no hay lotes, mostrar datos de ejemplo
    ...
    def on_lote_changed(self):
        ...
        try:
            # Verificar si hay lotes en el combo
            if self.lote_combo.count() == 0:
            ...
            lote_nombre = self.lote_combo.currentText()
            
            if not lote_id or lote_id == "--":
                print(f"No se pudo obtener el ID del lote seleccionado: {lote_id}")
                self.mostrar_datos_ejemplo()
                return
                
            print(f"Cargando seguimientos para lote: {lote_nombre} (ID: {lote_id})")
            ...
            if success:
                ...
                # Si no hay seguimientos, mostrar mensaje y datos de ejemplo
                if not seguimientos:
                    print(f"No hay seguimientos disponibles para el lote {lote_nombre} (ID: {lote_id})")
                    self.mostrar_datos_ejemplo()
                    return
                ...
                print(f"Se cargaron {len(seguimientos)} seguimientos para el lote {lote_nombre}")
            else:
