...
class LoteDialog(QDialog):
    ...
    def cargar_razas_y_galpones(self):
        ...
        try:
            ...
            success, razas = self.api_client.get_razas() if hasattr(self.api_client, 'get_razas') else (False, [])
            if success and razas and isinstance(razas, list):
                ...
                for raza in razas:
                    if isinstance(raza, dict):
                        self.raza_combo.addItem(str(raza.get('nombre', '')), raza.get('id'))
                    elif isinstance(raza, str):
                        # Si es una cadena, probablemente sea solo el nombre
                        self.raza_combo.addItem(raza, self.raza_combo.count() + 1)
                    else:
                        print(f"Tipo de raza no soportado: {type(raza)}")
            else:
                self.cargar_razas_ejemplo()
            
            # Cargar galpones desde la API
            success, galpones = self.api_client.get_galpones()
            if success and galpones and isinstance(galpones, list):
                # Guardar los datos completos para cada galpón
                self.galpones_data = galpones
                # Agregar nombres al combo
                for galpon in galpones:
                    if isinstance(galpon, dict):
                        self.galpon_combo.addItem(str(galpon.get('nombre', '')), galpon.get('id'))
                    elif isinstance(galpon, str):
                        # Si es una cadena, probablemente sea solo el nombre
                        self.galpon_combo.addItem(galpon, self.galpon_combo.count() + 1)
                    else:
                        print(f"Tipo de galpón no soportado: {type(galpon)}")
            else:
                self.cargar_galpones_ejemplo()
        except Exception as e:
            print(f"Error al cargar razas y galpones: {e}")
            self.cargar_datos_ejemplo()
    
    def cargar_datos_ejemplo(self):
        """Carga datos de ejemplo para razas y galpones"""
        self.cargar_razas_ejemplo()
        self.cargar_galpones_ejemplo()
    
    def cargar_razas_ejemplo(self):
        """Carga razas de ejemplo"""
        # Datos de ejemplo
        self.razas_data = [
            {'id': 1, 'nombre': 'Broiler'},
    ...
    def cargar_galpones_ejemplo(self):
        """Carga galpones de ejemplo"""
        # Datos de ejemplo
        self.galpones_data = [
    ...
    def cargar_datos_lote(self):
        ...
        try:
            ...
            self.nombre_input.setText(str(self.lote_data.get('nombre', '')))
            ...
            if fecha_str:
                try:
                    ...
                    fecha = QDate.fromString(str(fecha_str), "yyyy-MM-dd")
                    if not fecha.isValid():
                        fecha = QDate.fromString(str(fecha_str), "dd/MM/yyyy")
                    if not fecha.isValid():
                        fecha = QDate.fromString(str(fecha_str), "yyyy/MM/dd")
                    if fecha.isValid():
            ...
            # Cantidad
            try:
                cantidad = int(self.lote_data.get('cantidad_inicial', 100))
                self.cantidad_input.setValue(cantidad)
            ...
            elif isinstance(raza, (int, str)):
                # Puede ser solo el ID
                try:
                    raza_id = int(raza) if str(raza).isdigit() else raza
                    index = self.raza_combo.findData(raza_id)
                    if index >= 0:
                        self.raza_combo.setCurrentIndex(index)
                except Exception as e:
                    print(f"Error al procesar raza: {e}")
            ...
            elif isinstance(galpon, (int, str)):
                # Puede ser solo el ID
                try:
                    galpon_id = int(galpon) if str(galpon).isdigit() else galpon
                    index = self.galpon_combo.findData(galpon_id)
                    if index >= 0:
                        self.galpon_combo.setCurrentIndex(index)
                except Exception as e:
                    print(f"Error al procesar galpón: {e}")
            
            # Edad
            try:
                edad = int(self.lote_data.get('edad', 0))
                self.edad_input.setValue(edad)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir edad: {e}")
                self.edad_input.setValue(0)
            
            # Peso promedio
            try:
                peso = float(self.lote_data.get('peso_promedio', 0))
                self.peso_promedio.setValue(peso)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir peso promedio: {e}")
                self.peso_promedio.setValue(0)
            
            # Mortalidad
            try:
                mortalidad_diaria = int(self.lote_data.get('mortalidad_diaria', 0))
                self.mortalidad_diaria.setValue(mortalidad_diaria)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir mortalidad diaria: {e}")
                self.mortalidad_diaria.setValue(0)
                
            try:
                mortalidad_acumulada = int(self.lote_data.get('mortalidad_acumulada', 0))
                self.mortalidad_acumulada.setValue(mortalidad_acumulada)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir mortalidad acumulada: {e}")
                self.mortalidad_acumulada.setValue(0)
            
            # Producción
            try:
                produccion = int(self.lote_data.get('produccion_huevos', 0))
                self.produccion_huevos.setValue(produccion)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir producción de huevos: {e}")
                self.produccion_huevos.setValue(0)
                
            try:
                produccion_acumulada = int(self.lote_data.get('produccion_acumulada', 0))
                self.produccion_acumulada.setValue(produccion_acumulada)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir producción acumulada: {e}")
                self.produccion_acumulada.setValue(0)
            ...
            if fecha_seg_str:
                try:
                    # Intentar con diferentes formatos de fecha
                    fecha_seg = QDate.fromString(str(fecha_seg_str), "yyyy-MM-dd")
                    if not fecha_seg.isValid():
                        fecha_seg = QDate.fromString(str(fecha_seg_str), "dd/MM/yyyy")
                    if not fecha_seg.isValid():
                        fecha_seg = QDate.fromString(str(fecha_seg_str), "yyyy/MM/dd")
                    if fecha_seg.isValid():
                        self.fecha_ultimo_seguimiento.setDate(fecha_seg)
                except Exception as e:
                    print(f"Error al procesar fecha de último seguimiento: {e}")
            
            # Alimentación
            try:
                consumo = float(self.lote_data.get('consumo_alimento', 0))
                self.consumo_diario.setValue(consumo)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir consumo de alimento: {e}")
                self.consumo_diario.setValue(0)
                
            tipo_alimento = self.lote_data.get('tipo_alimento', '')
            if tipo_alimento:
                index = self.alimento_combo.findText(str(tipo_alimento))
                if index >= 0:
                    self.alimento_combo.setCurrentIndex(index)
            
            # Consumos
            try:
                consumo_agua = float(self.lote_data.get('consumo_agua', 0))
                self.consumo_agua.setValue(consumo_agua)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir consumo de agua: {e}")
                self.consumo_agua.setValue(0)
                
            try:
                consumo_energia = float(self.lote_data.get('consumo_energia', 0))
                self.consumo_energia.setValue(consumo_energia)
            except (ValueError, TypeError) as e:
                print(f"Error al convertir consumo de energía: {e}")
                self.consumo_energia.setValue(0)
                
        except Exception as e:
            print(f"Error general al cargar datos del lote: {e}")
            QMessageBox.warning(self, "Error", f"Ocurrió un error al cargar los datos del lote: {str(e)}\n\nSe cargarán valores por defecto.")
    ...
    def refresh_data(self):
        ...
        if success and lotes:
            ...
            if not isinstance(lotes, list):
                # Si no es una lista, podría ser un string u otro tipo de dato
            ...
            for row, lote in enumerate(lotes):
                ...
                # Nombre
                nombre_item = QTableWidgetItem(str(lote.get('nombre', '')))
                self.table.setItem(row, 1, nombre_item)
                
                # Fecha Ingreso
                fecha_item = QTableWidgetItem(str(lote.get('fecha_ingreso', '')))
